...
// console.log('set onResize')
      ...
      initFullpage: function() {
         ...
         $('#fullpage').fullpage({
			...
			normalScrollElements: '.wrap.too-high'
         });
      },
...
// console.log($(".micro-section.selected .wrap").height());
      },
      ...
      onMobileScroll: function(e) {
		 ...
		 if(this.checkIfTooTall()){
			 if(!this.sectionsAnimatedMobile){
				 this.i = 0;
				 this.loop();
				 this.sectionsAnimatedMobile = true;
			 }
...
// console.log(that.i + 1);
			...
			that.onLeave(that.i + 1);
			setTimeout(function(){ that.onAfterLoad(that.i + 1); }, FADE_OUT_SPEED + 100);

			if(that.i < that.$sections.length-1){
				that.loop();
			}
		 }, FADE_OUT_SPEED * that.i);
	  },
	   
	  checkIfTooTall: function(){
		  var heightOfSmallestSection = 10000;
		  this.$sections.each(function(){
			  if($(this).height() < heightOfSmallestSection){
				  heightOfSmallestSection = $(this).height();
			  }
		  });
		 
          if($('main').outerHeight() >= 2 * heightOfSmallestSection + 100){
			  return true;
		  }
		  else{
			  return false;
...
// console.log('checkIfRefreshNeeded CHECK');
